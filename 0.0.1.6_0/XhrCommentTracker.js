...
XhrCommentTracker.prototype = {
	...
	load: function() {
		var getUserLikesUnderStarterPromise = this.getLikesOfUserUnderStarter(),
			getUserFlagsUnderStarterPromise = this.getFlagsOfUserUnderStarter();
	...
	loadComments: function() {
		...
		.then(function getThreadPromises(response) {
			var promises = [], 
			...
			for (var i = 0; i < items.length; i++) {
				...
				for (var j = 0; j < baseCommentReplies.length; j++) {
					var reply = baseCommentReplies[j],
						...
						//parentId = reply.parentId,
						// replyParent = _this.commentsMap[parentId],
					...
					if (reply.authorIsStaff) {
						_this.staffCommentsMap.set(reply.id, reply);
					}
				...
				if (baseComment.authorIsStaff) {
					_this.staffCommentsMap.set(baseComment.id, baseComment);
				}
	...
	displayMoreComments: function(numOfComments) {
		...
		if (this.hidePending) {
			this.$commentList
				.find('article.reply--unapproved:not(.kinjamprove-followedUser)')
	...
	getPendingComments: function() {
		var pendingCommentsSelector = 'article.reply--unapproved:not(.kinjamprove-followedUser, [kinjamprove-hidden])',
